...
Both `phantom` and `phantom2` start with the same bait: a public GitHub repository that looks completely empty except for a single `README.md` and one visible commit on `main`. That naturally pushes us toward the local `.git` directory and standard Git forensics, but in both cases that path is intentionally barren.
...
There were later comments and other noisy artifacts that tried to redirect analysis:

* fake flags
* solver noise
* prompt-injection style comments aimed at LLMs

Those were not authoritative. The original author-attributed commit that directly added the flag was the strongest artifact and the one worth trusting.
...
At the same time, the public network graph exposed mostly noise. The main decoy was:

* commit `432c918b3792f029e9c106fe8999ad832dde4eee`
* branch `solve`
* file `solve.txt`
* contents `solve`

That commit was public solver noise, not the flag.
