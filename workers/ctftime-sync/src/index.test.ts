...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without secret → 401 Unauthorized", async () => {
  ...
  it("GET /sync with invalid secret → 401 Unauthorized", async () => {
    const res = await SELF.fetch("https://api.02labs.me/sync", {
  ...
  it("GET /sync with valid secret → always calls CTFtime API, returns fresh data, and updates KV", async () => {
