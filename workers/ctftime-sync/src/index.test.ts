...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without correct authorization → 401 Unauthorized", async () => {
    const res = await SELF.fetch("https://api.02labs.me/sync");
    expect(res.status).toBe(401);

    const resWithWrongToken = await SELF.fetch("https://api.02labs.me/sync", {
      headers: { Authorization: "Bearer wrong-secret" },
    });
    expect(resWithWrongToken.status).toBe(401);
  });

  it("GET /sync with correct authorization → always calls CTFtime API, returns fresh data, and updates KV", async () => {
