...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync → returns 401 if missing or invalid authorization", async () => {
    const resMissing = await SELF.fetch("https://api.02labs.me/sync");
    expect(resMissing.status).toBe(401);

    const resInvalid = await SELF.fetch("https://api.02labs.me/sync", {
      headers: { Authorization: "Bearer wrong-secret" },
    });
    expect(resInvalid.status).toBe(401);
  });

  it("GET /sync → always calls CTFtime API, returns fresh data, and updates KV when authorized", async () => {
