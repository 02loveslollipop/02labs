...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without token when secret is set → 401 Unauthorized", async () => {
  ...
  it("GET /sync with invalid token when secret is set → 401 Unauthorized", async () => {
    env.SYNC_SECRET = "test-secret";
    const res = await SELF.fetch("https://api.02labs.me/sync", {
  ...
  it("GET /sync with valid token → calls CTFtime API, returns fresh data, and updates KV", async () => {
    env.SYNC_SECRET = "test-secret";
