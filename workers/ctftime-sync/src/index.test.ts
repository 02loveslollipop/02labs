...
function makeFetchMock() {
  return vi.fn(
    (url: string | Request | URL, _init?: RequestInit): Promise<Response> => {
      const href = url instanceof Request ? url.url : url.toString();
      if (href.includes("/teams/")) {
        return Promise.resolve(
          new Response(JSON.stringify(MOCK_TEAM), { status: 200 }),
        );
      }
      if (href.includes("/results/")) {
        return Promise.resolve(
          new Response(JSON.stringify(MOCK_RESULTS), { status: 200 }),
        );
      }
...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync → fails securely when SYNC_SECRET is missing", async () => {
    // Using unit test strategy to bypass vitest-pool-workers binding immutability
    const req = new Request("https://api.02labs.me/sync");

    // Ensure syncData isn't accidentally hit, we just want to test the auth handler logic
    const localEnv = { ...env } as any;
    delete localEnv.SYNC_SECRET;

    // We import the worker object itself directly rather than using SELF for unit testing
  ...
  it("GET /sync → rejects missing or invalid credentials", async () => {
    // We use the binding from miniflare: "test-secret-from-miniflare"
    const noAuth = await SELF.fetch("https://api.02labs.me/sync");
    expect(noAuth.status).toBe(401);

    const wrongAuth = await SELF.fetch("https://api.02labs.me/sync", {
      headers: { Authorization: "Bearer wrong" },
    });
    expect(wrongAuth.status).toBe(401);
  });

  it("GET /sync → always calls CTFtime API, returns fresh data, and updates KV with valid auth", async () => {
    ...
    const res = await SELF.fetch("https://api.02labs.me/sync", {
      headers: { Authorization: "Bearer test-secret-from-miniflare" },
    });
    ...
    const stored = JSON.parse(
      (await env.CTFTIME_KV.get(KV_KEY))!,
    ) as CTFTimeData;
