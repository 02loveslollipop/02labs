...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync → 401 Unauthorized without correct secret", async () => {
    const resNoHeader = await SELF.fetch("https://api.02labs.me/sync");
    expect(resNoHeader.status).toBe(401);

    const resWrongSecret = await SELF.fetch("https://api.02labs.me/sync", {
