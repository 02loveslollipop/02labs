...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without valid token → 401 Unauthorized", async () => {
    const res1 = await SELF.fetch("https://api.02labs.me/sync"); // No token
    expect(res1.status).toBe(401);

    const res2 = await SELF.fetch("https://api.02labs.me/sync", {
      headers: { Authorization: "Bearer wrong-secret" },
    }); // Wrong token
    expect(res2.status).toBe(401);
  });

  it("GET /sync with valid token → always calls CTFtime API, returns fresh data, and updates KV", async () => {
