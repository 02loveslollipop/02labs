...
describe("ctftime-sync worker — local E2E", () => {
	...
	it("GET /sync (with secret configured) → 401 without correct Authorization header", async () => {
		const resNoAuth = await SELF.fetch("https://api.02labs.me/sync");
		expect(resNoAuth.status).toBe(401);

		const resWrongAuth = await SELF.fetch("https://api.02labs.me/sync", {
	...
	it("GET /sync (with secret configured) → works with correct Authorization header", async () => {
