...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without Authorization header → 401 Unauthorized", async () => {
    ...
    // Ensure CTFtime API was NOT called
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("GET /sync with invalid Authorization header → 401 Unauthorized", async () => {
    const res = await SELF.fetch("https://api.02labs.me/sync", {
  ...
  it("GET /sync with valid Authorization header → always calls CTFtime API, returns fresh data, and updates KV", async () => {
