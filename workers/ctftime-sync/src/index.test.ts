...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without auth → returns 401", async () => {
  ...
  it("GET /sync with invalid auth → returns 401", async () => {
    const res = await SELF.fetch("https://api.02labs.me/sync", {
  ...
  it("GET /sync with valid auth → always calls CTFtime API, returns fresh data, and updates KV", async () => {
