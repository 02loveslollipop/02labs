...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync without authorization → 401 Unauthorized", async () => {
  ...
  it("GET /sync with invalid authorization → 401 Unauthorized", async () => {
    const res = await SELF.fetch("https://api.02labs.me/sync", {
  ...
  it("GET /sync with valid authorization → always calls CTFtime API, returns fresh data, and updates KV", async () => {
