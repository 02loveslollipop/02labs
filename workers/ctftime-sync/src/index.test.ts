...
describe("ctftime-sync worker — local E2E", () => {
  ...
  it("GET /sync → fails with 401 without valid secret", async () => {
    const noAuthRes = await SELF.fetch("https://api.02labs.me/sync");
    expect(noAuthRes.status).toBe(401);

    const wrongAuthRes = await SELF.fetch("https://api.02labs.me/sync", {
